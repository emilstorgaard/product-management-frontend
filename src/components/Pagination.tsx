...
const Pagination: React.FC<PageProps> = ({ currentPage, totalPages, totalProducts, sort}) => {
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            {currentPageNumber > 1 ? (
                <Link href={`/products?page=${currentPageNumber-1}&sort=${sort}`} className="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-neutral-300 rounded-s hover:bg-neutral-400">
            ...
            {currentPageNumber < totalPagesNumber ? (
                <Link href={`/products?page=${currentPageNumber+1}&sort=${sort}`} className="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-neutral-300 border-0 border-s border-neutral-200 rounded-e hover:bg-neutral-400">
                    Next
